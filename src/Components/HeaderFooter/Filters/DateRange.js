...
function DateRange(props) {
  ...
  const [datePickerInvalid, setDatePickerInvalid] = useState(false)
  ...
  const textField = {
    '& .MuiInput-input':{
      color:datePickerInvalid ? 'red' :'white',
      fontSize:22,
      cursor:'pointer',
      paddingBottom:0,
      width:111
      ...
      >
        <div style={datePicker}>
          <DateRangePicker
            ...
            onChange={(newValue) => {
              if(newValue[0] && !isNaN(newValue[0]) && newValue[1] && !isNaN(newValue[1])){
                setDatePickerInvalid(false)
                dispatch(setFilter('date', {startDate: newValue[0], endDate: newValue[1]}))
              }else{
                setDatePickerInvalid(true)
              }
            }}
            renderInput={(startProps, endProps) => (
              <React.Fragment>
                <TextField {...startProps} InputProps={{ disableUnderline: true }} variant="standard" sx={textField} label="" />
                <Box sx={{ mx: 1 }}> - </Box>
                <TextField {...endProps} InputProps={{ disableUnderline: true }} variant="standard" sx={textField} label=""/>
              </React.Fragment>
            )}
          />
        </div>
        ...
        <Modal show={showModal === 'options'} style={{top:55}} paperStyle={{padding:0}} onClose={() => setShowModal('')} ignoreRef={optionsRef}>
          <div style={{width:295, display:'flex', flexDirection:'column'}}>
