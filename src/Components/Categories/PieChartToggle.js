...
const options = ['Actual', 'Planned']
const optionMap = {
  'Actual' : ACTUAL,
  'Planned' : PLANNED
}

function PieChartToggle(props) {
  const [isHover, setIsHover] = useState('')
  const [isButtonHover, setIsButtonHover] = useState(false)
  const activeOption = useSelector((state) => state.buttons.categoryChartType)
    ...
    >
        ...
        {options.map((option) => 
          <div 
            ...
            onClick={() => {dispatch(selectButton('categoryChartType', optionMap[option]));setIsHover('')}}
          >
            {option}
          </div>
        )}
